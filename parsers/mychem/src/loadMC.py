...
# By: Phil Owen
# Date: 4/5/2021
# Desc: Class that loads/parses the  data.
##############
class MCLoader(SourceDataLoader):
    ...
    def __init__(self, test_mode: bool = False):
        ...
        self.source_id: str = ''
        self.source_db: str = 'MyChem'

        # create a logger
        self.logger = LoggingUtil.init_logging("Data_services.mychem.MCLoader", level=logging.INFO, line_format='medium', log_file_path=os.environ['DATA_SERVICES_LOGS'])
    ...
    def get_provenance(self) -> dict:
        ...
        """
        # create the record
        provenance: dict = {'attribute_type_id': 'biolink:original_knowledge_source', 'value': 'infores:mychem'}
    ...
    def get_mychem_data(self) -> int:
        """
        Gets the mychem data.
    ...
    def load(self, nodes_output_file_path: str, edges_output_file_path: str) -> dict:
        """
        parses the MyChem data file gathered from
        ...
        self.logger.info(f'MyChem - Start of  data processing.')

        # get the list of taxons to process
        file_count = self.get_mychem_data()
        ...
        if file_count == 1:
            ...
            self.logger.info(f'MyChem - {self.data_file} Processing complete.')

            # write out the data
            self.write_to_file(nodes_output_file_path, edges_output_file_path)

            self.logger.info(f'MyChem - Processing complete.')
        else:
...
if __name__ == '__main__':
    ...
    ap = argparse.ArgumentParser(description='Load MyChem data files and create KGX import files.')

    ap.add_argument('-r', '--data_dir', required=True, help='The location of the MyChem data file')
