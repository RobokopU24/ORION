...
class BLLoader(SourceDataLoader):
    ...
    def get_provenance(self) -> dict:
        ...
        """
        # create the record
        provenance: dict = {'attribute_type_id': 'biolink:original_knowledge_source', 'value': 'infores:'}
    ...
    def load(self, nodes_output_file_path: str, edges_output_file_path: str) -> dict:
        """
        parses the biolink data file gathered
        ...
        self.logger.info(f'BLLoader - Start of biolink model data processing.')

        # get the list of taxons to process
        file_count = self.get_biolink_data()
        ...
        if file_count == 1:
            self.logger.debug(f'{self.data_file} archive retrieved. Parsing Biolink data.')

            # parse the data
            load_metadata = self.parse_data_file(self.data_path, self.data_file)

            self.logger.info(f'BLLoader - {self.data_file} Processing complete.')

            # write out the data
            self.write_to_file(nodes_output_file_path, edges_output_file_path)

            self.logger.info(f'BLLoader - Processing complete.')
        else:
            self.logger.error(f'Error: Retrieving BLLoader archive failed.')
