...
##############
# Class: ChemNorm loader
#
# By: Phil Owen
# Date: 4/5/2021
# Desc: Class that loads/parses the ChemNorm data.
##############
class CNLoader(SourceDataLoader):
    ...
    def __init__(self, test_mode: bool = False):
        ...
        # create a logger
        self.logger = LoggingUtil.init_logging("Data_services.ChemNorm.CNLoader", level=logging.INFO, line_format='medium', log_file_path=os.environ['DATA_SERVICES_LOGS'])
    ...
    def get_provenance(self) -> dict:
        ...
        """
        # create the record
        provenance: dict = {'attribute_type_id': 'biolink:original_knowledge_source', 'value': 'infores:chemnorm'}
    ...
    def get_chemnorm_data(self) -> int:
        """
    ...
    def load(self, nodes_output_file_path: str, edges_output_file_path: str) -> dict:
        """
        parses the ChemNorm data file gathered
        ...
        self.logger.info(f'ChemNorm - Start of  data processing.')

        # get the list of taxons to process
        file_count = self.get_chemnorm_data()
        ...
        if file_count == 1:
            ...
            self.logger.info(f'ChemNorm - {self.data_file} Processing complete.')

            # write out the data
            self.write_to_file(nodes_output_file_path, edges_output_file_path)

            self.logger.info(f'ChemNorm - Processing complete.')
        else:
            self.logger.error(f'Error: Retrieving ChemNorm archive failed.')
...
if __name__ == '__main__':
    ...
    ap = argparse.ArgumentParser(description='Load ChemNorm data files and create KGX import files.')

    ap.add_argument('-r', '--data_dir', required=True, help='The location of the ChemNorm data file')
