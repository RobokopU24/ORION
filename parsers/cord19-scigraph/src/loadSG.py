...
##############
# Class: Cord19-SciGraph loader
#
# By: Phil Owen
# Date: 4/5/2021
# Desc: Class that loads/parses the Cord19-SciGraph data.
##############
class SGLoader(SourceDataLoader):
    ...
    def get_provenance(self) -> dict:
        ...
        """
        # create the record
        provenance: dict = {'attribute_type_id': 'biolink:original_knowledge_source', 'value': 'infores:cord19-scigraph'}
    ...
    def get_cord19_scigraph_data(self) -> int:
        """
        Gets the Cord19-SciGraph data.
    ...
    def load(self, nodes_output_file_path: str, edges_output_file_path: str) -> dict:
        """
        parses the Cord19-SciGraph data file gathered
        ...
        self.logger.info(f' - Start of Cord19-SciGraph data processing.')

        # get the list of taxons to process
        file_count = self.get_cord19_scigraph_data()
        ...
        if file_count == 1:
            ...
            self.logger.info(f'Cord19-SciGraph - {self.data_file} Processing complete.')

            # write out the data
            self.write_to_file(nodes_output_file_path, edges_output_file_path)

            self.logger.info(f'Cord19-SciGraph - Processing complete.')
        else:
            self.logger.error(f'Error: Retrieving Cord19-SciGraph archive failed.')
...
if __name__ == '__main__':
    ...
    ap.add_argument('-r', '--data_dir', required=True, help='The location of the Cord19-SciGraph data file')
