...
##############
# Class: ChemBio loader
#
# By: Phil Owen
# Date: 4/5/2021
# Desc: Class that loads/parses the ChemBio data.
##############
class CBLoader(SourceDataLoader):
    ...
    def __init__(self, test_mode: bool = False):
        ...
        self.source_id: str = ''
        self.source_db: str = 'ChemBio'

        # create a logger
        self.logger = LoggingUtil.init_logging("Data_services.ChemBio.CBLoader", level=logging.INFO, line_format='medium', log_file_path=os.environ['DATA_SERVICES_LOGS'])
    ...
    def get_provenance(self) -> dict:
        ...
        """
        # create the record
        provenance: dict = {'attribute_type_id': 'biolink:original_knowledge_source', 'value': 'infores:chembio'}
    ...
    def get_chembio_data(self) -> int:
        """
        Gets the ChemBio data.
    ...
    def load(self, nodes_output_file_path: str, edges_output_file_path: str) -> dict:
        """
        parses the bioling data file gathered from
        ...
        self.logger.info(f'CBLoader - Start of ChemBio data processing.')

        # get the list of taxons to process
        file_count = self.get_chembio_data()
        ...
        if file_count == 1:
            ...
            self.logger.info(f'ChemBio - {self.data_file} Processing complete.')

            # write out the data
            self.write_to_file(nodes_output_file_path, edges_output_file_path)

            self.logger.info(f'ChemBio - Processing complete.')
        else:
            self.logger.error(f'Error: Retrieving ChemBio archive failed.')
...
if __name__ == '__main__':
    ...
    ap = argparse.ArgumentParser(description='Load ChemBio data files and create KGX import files.')

    ap.add_argument('-r', '--data_dir', required=True, help='The location of the ChemBio data file')
