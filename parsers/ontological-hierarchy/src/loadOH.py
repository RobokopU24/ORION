...
class OHLoader(SourceDataLoader):
    ...
    def get_provenance(self) -> dict:
        ...
        """
        # create the record
        provenance: dict = {'attribute_type_id': 'biolink:original_knowledge_source', 'value': 'infores:ontological-hierarchy'}
    ...
    def get_ontological_hierarchy_data(self) -> int:
        """
    ...
    def load(self, nodes_output_file_path: str, edges_output_file_path: str) -> dict:
        """
        parses the Ontological-Hierarchy data file gathered from
        ...
        self.logger.info(f'Ontological-Hierarchy - Start of  data processing.')

        # get the list of taxons to process
        file_count = self.get_ontological_hierarchy_data()
        ...
        if file_count == 1:
            ...
            self.logger.info(f'Ontological-Hierarchy - {self.data_file} Processing complete.')

            # write out the data
            self.write_to_file(nodes_output_file_path, edges_output_file_path)

            self.logger.info(f'Ontological-Hierarchy - Processing complete.')
        else:
