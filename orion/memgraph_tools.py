...
def create_memgraph_dump(nodes_filepath: str,
    ...
    try:
        if not os.path.exists(output_csv_node_file):
            logger.info(f'Creating memgraph node csv for {graph_id}({release_version})...')
        ...
        if not os.path.exists(output_cypher_node_idx_file):
            logger.info(f'Creating memgraph index cypher for {graph_id}({release_version})...')
        ...
        if not os.path.exists(output_edge_manifest):
            logger.info(f'Creating memgraph edge csvs for {graph_id}({release_version})...')
        ...
        return False
    logger.info(f'Memgraph dump files created for {graph_id}({release_version}).')
    return True
