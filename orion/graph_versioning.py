...
- **build_version** — a deterministic ``xxh64`` hash of the graph's inputs
  (data source versions, normalization schemes, merge strategies, subgraph
  release versions). Two builds with identical inputs share a build_version,
  so this is what ORION uses to tell whether a graph actually changed.
