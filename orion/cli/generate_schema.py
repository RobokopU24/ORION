...
def main():
    ...
    ap.add_argument('--graph-base-url', default='',
                    help="url the graph's files are published under, used as the graph @id and as "
                         "the prefix for the schema's @id")
    ...
    if not args.graph_base_url:
        print('No --graph-base-url given, so the schema cannot identify the graph it describes. ',
              file=sys.stderr)

    generate_kgx_schema_file(nodes_filepath=args.nodes_filepath,
                             edges_filepath=args.edges_filepath,
                             output_dir=output_dir,
                             graph_output_url=args.graph_base_url,
