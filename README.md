...
- **Edge ids become the relationship `:ID` column.** Edges built with `add_edge_id` keep their own ids; otherwise they're numbered sequentially. Ids from the file are used only when every edge has one — if only some do, all of them are numbered, since Neptune reads two edges sharing an `:ID` as one edge. Either way the load provides the ids, which lets a failed load resume instead of reloading every relationship. Pass `--no_generate_edge_ids` to `orion-neptune-dump` to leave the column out and let Neptune assign the ids.
